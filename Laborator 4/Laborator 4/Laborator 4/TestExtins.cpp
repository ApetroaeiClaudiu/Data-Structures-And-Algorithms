...
void testCreeaza() {
	...
	IteratorContainer im = m.iterator();
...
void testSterge() {
	...
	v = m.cauta(0);
	assert(v.size() == 0);
	IteratorContainer it = m.iterator();
	it.urmator();
...
void testIterator() { // nu stim reprezentarea MD, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
	MD m;
	IteratorContainer im = m.iterator(); //iterator pe MD vid
	assert(im.valid() == false);

	for (int i = 0; i < 100; i++) {  //adaug 100 de elemente, fiecare e cheia si valoarea 33
		m.adauga(33, 33);
	}
	IteratorContainer im2 = m.iterator(); //daca iterez doar 33 poate sa-mi dea iteratorul
	...
	IteratorContainer im3 = m2.iterator();
	...
	IteratorContainer im4 = m3.iterator();
	...
	IteratorContainer im5 = m4.iterator();
	while (im5.valid()) {
		TElem e = im5.element();
